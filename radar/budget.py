...
只保护付费调用：DeepSeek 摘要 / DeepSeek 精读 / twitterapi.io 推文。
账本按本地日期自动归零；线程安全(warm 用线程池并发摘要/推文)。
价格表与真实计费收口在 lib/llm.py，此处只管预留额度。
"""
...
# 单次调用的成本估算(USD)，用于预留；之后按 llm.chat 返回的真实花费结算
EST_SUMMARY = 0.0006      # v4-flash 短摘要
EST_DEEP = 0.006          # 精读(正文可达 2 万字，仍不过几厘)
EST_TWEET = 0.003
